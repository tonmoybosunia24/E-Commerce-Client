...
const useProducts = (page, sortBy, filter, searchInput) => {
       ...
       const { data = {}, isPending: allProductsLoading } = useQuery({
              ...
              queryFn: async () => {
                     ...
                     const res = await axiosPublic.get(`/products?page=${page}&sort=${sortBy}&limit=${limit}&stockFilter=${stockFilter}&size=${size}&brands=${brands}&search=${searchInput}&minPrice=${minPrice}&maxPrice=${maxPrice}&category=${category}`);
                     return res.data;
              }
       })
