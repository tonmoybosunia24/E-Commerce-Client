       ...
       return (
              <section className="px-5 md:px-10 lg:px-20 my-5 md:my-7 lg:my-10">
                     ...
                     {productsLoading ? (<span className="loading loading-spinner text-error flex items-center m-auto min-h-screen"></span>) : (<Swiper
                            ...
                            className={`mySwiper h-auto ${!productsLoading && products.length >= 5 ? 'border border-l-0 border-Radical' : ''} ${products.length > 1 && products.length < 5 ? 'border-l border-gray-400' : ''}`}
                     >
                            {products.map((product, productIndex) => (
                                   <SwiperSlide key={productIndex} className={`!h-auto select-text flex ${productIndex === 0 ? 'border-l border-Radical' : 'border-l border-Radical'} ${products.length > 0 && products.length < 5 ? 'border-t border-r border-b' : ''} ${products.length > 1 && products.length < 5 ? 'border-l-0' : ''}`}>
                                          <div className="w-full h-full flex flex-col">
                                                 <ProductCard fromSlider="isOnSale" product={product} />
                                          </div>
                                   </SwiperSlide>
                            ))}
                     </Swiper>
