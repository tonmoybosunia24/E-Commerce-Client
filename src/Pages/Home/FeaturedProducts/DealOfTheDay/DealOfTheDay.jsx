...
const DealOfTheDay = () => {
       ...
       const [products, loading] = useCategoryProducts('?isOnSale=true')

       return (
              <section className="px-5 md:px-10 lg:px-20 my-5 md:my-7 lg:my-10">
                     {/* -----------------Title And Slider Navigation----------------- */}
                     <div className='flex justify-between items-center mb-3'>
                            <h2 className='text-base md:text-xl lg:text-xl hover:text-Radical cursor-default'>Deal of the day</h2>
                     ...
                     <div className='h-auto'>
                            <Swiper
                                   ...
                                   className={`mySwiper h-auto ${!loading && products.length >= 5 ? 'border border-Radical' : ''} ${products.length > 1 && products.length < 5 ? 'border-l border-gray-400' : ''}`}
                                   breakpoints={{
                                          640: { slidesPerView: 2 },
                                          768: { slidesPerView: 4 },
