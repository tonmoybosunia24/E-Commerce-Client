...
const CategoryProducts = () => {
    ...
    const [products, productsLoading] = useCategoryProducts(`?Category=${activeCategory}`);
            ...
            {categoriesLoading ? (<span className="loading loading-spinner text-error flex items-center m-auto min-h-screen"></span>) : (
                <Tabs onSelect={(index) => {
                    ...
                    <div className='flex justify-between items-center mb-3'>
                        <TabList className="flex gap-3 relative active:border-none text-base md:text-lg lg:text-lg overflow-x-auto lg:overflow-visible max-w-full">
                            {categories.map((category, index) => (
                                <Tab key={index} className={category === activeCategory ? "font-bold outline-none whitespace-nowrap cursor-pointer" : "font-semibold text-black whitespace-nowrap cursor-pointer"}>{category}</Tab>
                            ))}
                        </TabList>
                        {/* ------------------Category Navigation----------------- */}
