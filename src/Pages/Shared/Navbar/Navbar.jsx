       ...
       return (
              <nav className={`hidden lg:flex justify-between sticky top-0 bg-white px-5 md:px-10 lg:px-20 py-3 border-b transition-shadow duration-100 ease-in-out border-gray-300 ${shadow ? 'shadow-md' : ''} relative z-30`}>
                     ...
                     <div className="relative">
                            ...
                            {/* -----------------All Categories----------------- */}
                            <ul tabIndex={0} className={`absolute left-0 z-10 w-full p-2 shadow-md mt-3 transition-all delay-75 duration-500 ease-in-out bg-white ${drawerOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-5"} border border-gray-300 space-y-1`}>{categories.map((category, index) => (
