...
const UpdateProducts = () => {
       ...
       const { id } = useParams();
       const [updateProducts, updateProductsLoading] = useUpdateProducts(id);
       const [tags, setTags] = useState([]);
       const [colors, setColors] = useState([]);
       const [variant, setVariant] = useState([]);
       const { uploadImages } = useUploadImage();
       const { register, handleSubmit, setValue, formState: { errors, isValid }, reset } = useForm({ mode: 'onChange' });
       const { Title, Description, Category, SubCategory, Price, OfferPrice, DiscountPercentage, Rating: ProductRating, Stock, Brand, Images, Colors, Variant, ShippingInformation, Condition, AvailabilityStatus, SellerInformation } = updateProducts;
                                   ...
                                   <form onSubmit={handleSubmit(onsubmit)} className="card-body flex items-center p-0">
                                          <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                                 ...
                                                 <div className="form-control space-y-1.5">
                                                        ...
                                                        <select required defaultValue={Category} {...register('Category', { required: true })} className="w-full select select-bordered focus:outline-0">
